...
async def handle_chat(event: MessageEvent, matcher: Matcher):
    ...
    try:
        ...
        is_split_mode = session_mgr.split_enabled
        input_tokens = None
        output_tokens = None
...
async def handle_banwords_list(event: MessageEvent):
    if str(event.user_id) not in get_admin_ids():
...
async def handle_reload_persona(event: MessageEvent):
    ...
    try:
        aichat_config.reload_personas()
        await reload_persona_cmd.finish(
            f"人设配置已重新加载！公开{len(aichat_config.PERSONA_CATALOG)}个，隐藏{len(aichat_config.HIDDEN_PERSONA_CATALOG)}个")
    except Exception as e:
...
async def handle_switch_persona(event: MessageEvent, args: Message = CommandArg()):
    ...
    pk = arg
    if not pk.isdigit():
        hw = aichat_config.HIDDEN_PERSONA_WHITELISTS.get(pk, set())
    ...
    p = aichat_config.HIDDEN_PERSONA_CATALOG[pk] if not pk.isdigit() else aichat_config.PERSONA_CATALOG[int(pk)]
...
async def handle_switch_global_persona(event: MessageEvent, args: Message = CommandArg()):
    ...
    p = aichat_config.PERSONA_CATALOG[pno]
...
async def handle_switch_group_persona(event: MessageEvent, args: Message = CommandArg()):
    ...
    if sender_role not in ("owner", "admin") and user_id not in get_admin_ids():
        await switch_group_persona_cmd.finish("只有群主/群管理员才能切换群人设。")
    arg = args.extract_plain_text().strip()
    if not arg.isdigit():
        await switch_group_persona_cmd.finish("用法：/切换群人设 + 数字")
    pno = int(arg)
    if pno not in aichat_config.PERSONA_CATALOG:
...
async def handle_split(event: MessageEvent, args: Message = CommandArg()):
    ...
    reply_map = {("开", "on", "启用"): ("已全局启用分句模式", True),
                 ("关", "off", "禁用"): ("已全局禁用分句功能", False)}
    for keywords, (msg, status) in reply_map.items():
        if arg in keywords:
            session_mgr.set_split(status)
            await split_control.send(f"{msg}（已持久化）"); return
    current = session_mgr.split_enabled
    await split_control.send(f"当前全局分句状态：{'✅ 启用' if current else '❌ 禁用'}\n使用 /切分 开/关 控制")
